...
exports.run = async (event, context) => {
  ...
  const getText = () => {
    try {
      return document.querySelector('.widget.reports .js-report-avg-value').textContent;
    } catch (e) {
      return '';
    }
  }
  ...
  if (!text) {
    return Promise.reject('hrm, could not parse step count, probs gotta update somethin');
  }
